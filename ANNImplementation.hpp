    ...
    VectorOfSizeSix * const particleVirial) const
{
  ...
  double ** dEdr = nullptr;  // saving all the gradients
  if (need_dEdr)
  {
    AllocateAndInitialize2DArray<double>(dEdr, Nparticles, Nparticles);
  }
  ...
  for (int i = 0; i < Nparticles; i++)
  {
    ...
    if (need_dE)
    {
      ...
      AllocateAndInitialize3DArray<double>(
          dGCdr_ij, Ndescriptors, numnei, numnei);
    }
    ...
    if (descriptor_->need_normalize())
    {
      for (int j = 0; j < Ndescriptors; j++)
      {
        ...
        if (need_dEdr)
        {
          ...
          {
            for (int l = 0; l < numnei; l++) { dGCdr_ij[j][k][l] /= std; }
          }
        }
      }
    }
    ...
    if (need_dE == true || need_dEdr == true)
    {
      for (int j = 0; j < Ndescriptors; j++)
      {
    ...
    for (int ii = 0; ii < Nparticles; ii++)
    {
      for (int jj = 0; jj < ii; jj++)
      {
        const std::array<double, 3> r_i
            = {coordinates[jj][0] - coordinates[ii][0],
               coordinates[jj][1] - coordinates[ii][1],
