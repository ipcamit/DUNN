...
// grad_dr_desc is 3D array of shape numDesc * Nparticles * Nparticles
//*****************************************************************************
  ...
  for (int jj = 0; jj < numnei; ++jj)
  {
    ...
    for (size_t p = 0; p < name_.size(); p++)
    {
      ...
      for (int q = 0; q < num_param_sets_[p]; q++)
      {
        ...
        if (name_[p] == "g1")
        {
          if (grad || grad_dr) { sym_d_g1(rijmag, rcutij, gc, dgcdr_two); }
          else
          {
        ...
        else if (name_[p] == "g2")
        {
          ...
          if (grad || grad_dr) { sym_d_g2(eta, Rs, rijmag, rcutij, gc, dgcdr_two); }
          else
          {
        ...
        else if (name_[p] == "g3")
        {
          ...
          if (grad || grad_dr) { sym_d_g3(kappa, rijmag, rcutij, gc, dgcdr_two); }
          else
          {
